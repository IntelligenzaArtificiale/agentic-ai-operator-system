...
Installa **Agentic AI Operator System 2.0.0** su Windows 10/11 x64.
...
4. Leggi `INSTALL_RESULT.json`. Non dichiarare successo se `ok`, `mcp_configured`, `plugin_installed`, `opensteps_installed`, `updater_installed` e `dashboard_ready` non sono tutti `true`.
5. Verifica inoltre `codex mcp get windows-mcp` e la presenza di `automazione-totale-procedure@personal` in `codex plugin list`.
