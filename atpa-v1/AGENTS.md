...
4. Leggi `INSTALL_RESULT.json`. Non dichiarare successo se `ok`, `mcp_configured`, `runner_configured`, `plugin_installed`, `opensteps_installed`, `updater_installed`, `dashboard_ready` e `company_profile_ready` non sono tutti `true`.
...
7. Concludi: “Il sistema è pronto. Inizia con `$start-agentic-ai-operator-system`, poi configura il DNA con `$profilo-azienda`.”
