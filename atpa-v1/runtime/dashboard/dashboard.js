(() => {
  ...
  function renderSystem() {
    ...
    const items = [['Procedure',data.counts.total],['Esecuzioni',data.counts.runs],['Successi',data.counts.successful_runs],['Errori',data.counts.incidents],['Tempo medio',duration(data.counts.average_duration_ms)]];
  ...
  function showDetail(slug) {
    ...
    byId('detailBody').innerHTML = `<div class="brand">${escapeHtml(meta.category||'Procedura')}</div><h2>${escapeHtml(meta.name)}</h2><p>${escapeHtml(meta.description)}</p><p><b>Reparto:</b> ${escapeHtml(meta.department||'—')} · <b>Versione:</b> ${escapeHtml(meta.version)}</p><h3>Prestazioni</h3><table class="metric-table"><tr><th>Esecuzioni</th><td>${item.metrics.run_count||0}</td><th>Successi</th><td>${item.metrics.successful_runs||0}</td></tr><tr><th>Media</th><td>${duration(item.metrics.average_duration_ms)}</td><th>Migliore</th><td>${duration(item.metrics.best_duration_ms)}</td></tr><tr><th>Errori</th><td>${item.metrics.incident_count||0}</td><th>Ultima</th><td>${duration(item.metrics.last_duration_ms)}</td></tr></table><h3>Step più lenti</h3><table class="metric-table">${slow.length?slow.map(step=>`<tr><td>${escapeHtml(step.label)}</td><td>${duration(step.average_duration_ms)}</td><td>${step.samples} campioni</td></tr>`).join(''):'<tr><td>Nessun campione disponibile.</td></tr>'}</table><h3>Flusso operativo</h3><div class="flow">${nodes.length?nodes.map(node=>`<div class="node ${node.type==='condition'?'condition':''}"><b>${escapeHtml(node.label)}</b><br><small>${escapeHtml(node.description||'')}</small></div>`).join(''):'<span class="empty">Diagramma non ancora definito.</span>'}</div><p><a href="${item.folder_uri}">Apri cartella procedura</a></p>`;
