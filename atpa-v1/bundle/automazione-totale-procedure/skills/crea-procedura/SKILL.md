...
4. Genera `procedure.json`, `execution-plan.json`, `SKILL.md`, `references\procedure.md`, `references\test-cases.md`, `references\screenshots\`, `tests\`, `runs\`, `experience\errors.jsonl` e `experience\lessons.json`. Parti dagli schemi in `TEMPLATE-PROCEDURA`. Assegna a ogni nodo operativo un `id` stabile e `required`; compila `experience_context` con applicazioni, reparto, categoria, pattern e riferimenti condivisi realmente pertinenti.
