...
Prima di iniziare, chiama `windows-mcp.LicenseStatus`. Se la licenza non è attiva, non raccogliere né creare procedure e usa soltanto `windows-mcp.OpenLicenseActivation`; non chiedere mai la chiave in chat.
...
2. Genera e completa `procedure.json`, `execution-plan.json`, `SKILL.md`, `references\procedure.md`, `references\test-cases.md`, `references\screenshots\`, `tests\`, `runs\`, `experience\errors.jsonl`, `experience\lessons.json` e `CHANGELOG.md`. La cartella `recording\` può essere assente o vuota: la procedura non deve dipendere da una registrazione. Assegna a ogni nodo operativo un `id` stabile e `required`; compila `experience_context` con applicazioni, reparto, categoria, pattern e riferimenti condivisi pertinenti.
