...
try{
    ...
    $run1=@{schema_version=1;run_id='run-1';outcome='succeeded';duration_ms=86000;steps=@(@{step_id='compose';label='Compila email';duration_ms=18000})}
    $run2=@{schema_version=1;run_id='run-2';outcome='succeeded';duration_ms=21600;steps=@(@{step_id='compose';label='Compila email';duration_ms=6500})}
    ...
    if($data -notmatch '"runs":2'){throw "Conteggio run non corretto. Data: $data"}
    if($data -notmatch '"incidents":1'){throw 'Conteggio incidenti non corretto.'}
    if($data -notmatch '"best_duration_ms":21600'){throw 'Durata migliore non corretta.'}
