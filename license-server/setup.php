...
?>
<!doctype html><html lang="it"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Setup licenze AIOS</title><link rel="stylesheet" href="assets/admin.css"></head><body><main class="shell"><section class="login"><h1>Setup licenze</h1><?php if ($error): ?><p class="notice error"><?= htmlspecialchars($error) ?></p><?php endif; ?><?php if ($completed): ?><p class="notice success">Configurazione completata. Elimina setup.php dal server e apri index.php.</p><?php else: ?><form method="post"><label>Token monouso<input type="password" name="setup_token" minlength="32" required autofocus></label><label>Nuova password amministratore<input type="password" name="password" minlength="14" required></label><button>Configura in sicurezza</button></form><?php endif; ?></section></main></body></html>
