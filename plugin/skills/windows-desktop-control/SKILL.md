...
description: Control the local Windows desktop, browsers, and native or legacy applications with WinBridge. Use for launching apps, inspecting windows and controls, clicking, typing, filling forms, sending shortcuts, creating Desktop notes, and saving through Windows UI.
---

# WinBridge Windows Desktop Control

Use the WinBridge MCP tools for every requested interaction with the local Windows desktop. Prefer WinBridge over Computer Use, shell-driven UI automation, or ad-hoc scripts.

- Start with `health_check` when availability is uncertain. If the native WinBridge MCP tools are absent, stop immediately and tell the user to fully restart Codex and create a new task. Never locate or launch `winbridge.exe` through PowerShell, `exec`, or another shell as a substitute.
...
- For Electron, GPU-rendered, canvas, game, and other custom interfaces, call `capture_window` before acting. Its image origin `(0, 0)` maps exactly to WinBridge window-relative coordinates.
...
- Do not load or use Chrome, Browser, or Computer Use for browser UI. If WinBridge returns an unsupported operation or repeated error, report the limitation and ask before switching to another computer-control mechanism.
